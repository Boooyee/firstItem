...
angular.module('admin').directive('sidebar', function () {
  return {
    ...
    controller: [
      ...
      'recordCookies',
      'commonUtil',
      function ($scope,$timeout,httpService, $rootScope, $state, $location, roleService, managerService, moduleService, recordCookies, commonUtil) {
          ...
          if (res.data.code == 0) {
            ...
            if (sessionStorage.mineSide == '' || sessionStorage.mineSide == undefined) {
              moduleService.batchGetModule(moduleIDs).then(function (res) {
                if (res.data.code == 0) {
                  //$scope.sideList = res.data.data;            //写入的侧边栏数据
                  $scope.sideList = commonUtil.buildTree(res.data.data.moduleList);  // 刷新时的高亮标识
                  //console.log($scope.sideList);                                                 //markLightFromUrl();
                }
              });
            } else {
    ...
    setInterval(function(){
      httpService.sidebarRedMark().then(function(res){
        $scope.taskNum = res.data.data.count;
      })
    },30000)
